...
public class Instantiation implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		EntryLog log1 = new EntryLog(null, LocalDateTime.now(), "Infoplem", "PC", EntryStatus.GRANTED);
		EntryLog log2 = new EntryLog(null, LocalDateTime.now(), "Renner", "Roupas", EntryStatus.DENIED);
		
		log1.getDrivers().add(driver1);
		log1.getAssistants().add(assistant1);
		log1.getVehicles().add(vehicle2);
		
		log2.getDrivers().addAll(Arrays.asList(driver2, driver3));
		log2.getAssistants().addAll(Arrays.asList(assistant2, assistant3));
		log2.getVehicles().addAll(Arrays.asList(vehicle1, vehicle3));

		entryLogRepository.saveAll(Arrays.asList(log1, log2));
